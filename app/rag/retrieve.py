...
def _lexical_overlap_score(query: str, content: str) -> float:
    ...
    whole_query_bonus = 0.1 if _normalize_text(query).lower() in normalized_content else 0.0
...
def _segment_content(content: str) -> list[str]:
    ...
    segments = [_normalize_text(segment) for segment in raw_segments]
...
def _semantic_rerank_items(query: str, items: list[dict]) -> list[dict]:
    ...
    try:
        ...
        _logger.exception("rag.retrieve semantic rerank failed query=%s", query_text)
        return items
